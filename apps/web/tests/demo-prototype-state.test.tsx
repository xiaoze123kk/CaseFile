...
describe("demo prototype state model", () => {
  it("deterministically generates three distinct, reference-complete workbench seeds", () => {
    ...
    const first = buildPrototypeDraftCandidates(input, 3);
    const second = buildPrototypeDraftCandidates(input, 3);
  ...
  it("keeps an adopted workbench valid while making non-adopted old candidates stale", () => {
    ...
    let state: DemoPrototypeState = {
      ...createInitialDemoPrototypeState(),
      brief,
      review,
    };

    state = demoPrototypeReducer(state, { type: "freeze_review" });
    expect(state.frozenBriefVersion).toBeNull();
    state = demoPrototypeReducer(state, { type: "save_review" });
    state = demoPrototypeReducer(state, { type: "freeze_review" });
    ...
    state = demoPrototypeReducer(state, {
    ...
    expect(prototypeDraftCandidateStatus(state, candidates[0])).toBe("current");

    state = demoPrototypeReducer(state, { type: "begin_revision" });
    expect(state.workingBriefVersion).toBe(2);
    expect(state.frozenBriefVersion).toBeNull();
    expect(state.adoptedCandidateId).toBe(candidates[0].id);
    expect(prototypeDraftCandidateStatus(state, candidates[0])).toBe("current");
    expect(prototypeDraftCandidateStatus(state, candidates[1])).toBe("stale");

    const unchanged = demoPrototypeReducer(state, {
      type: "adopt_candidate",
      candidateId: candidates[1].id,
    });
    expect(unchanged.adoptedCandidateId).toBe(candidates[0].id);
    expect(
      demoPrototypeReducer(state, {
...
function ProviderProbe() {
  const { state, patchState } = useDemoPrototype();
  return (
    <button
...
describe("DemoPrototypeProvider lifecycle", () => {
