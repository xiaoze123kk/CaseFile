...
import type { PrototypeDraftCandidate } from "@/features/analyst-workbench/analyst-fixture";
import type {
  PrototypeAnswer,
  PrototypeBrief,
...
export function mapAnswersFromQuestions(
  questions: BriefIntakeQuestionView[],
): Record<string, PrototypeAnswer> {
  const answers: Record<string, PrototypeAnswer> = {};
...
export function mapCandidateView(
  ...
  const originLabel: Record<PrototypeCandidate["origin"], string> = {
    agent: "方向核验初稿",
    manual: "人工简报起点",
    dialogue: "对话修改候选",
  };
  const origin: PrototypeCandidate["origin"] =
...
export function mapIntakeToDemoState(intake: BriefIntakeView) {
...
export function mapDraftCandidateView(
  view: DraftCandidateView,
  base: PrototypeDraftCandidate,
