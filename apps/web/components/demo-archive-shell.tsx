...
function isDemoModuleActive(pathname: string, href?: string) {
  if (!href) return false;
  if (href === "/demo") {
    return pathname === "/demo" || pathname === "/demo/brief";
  }
  if (href.startsWith("/demo/quality")) {
    return pathname === "/demo/quality";
  }
...
export function DemoArchiveShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div
      className="archive-app demo-template"
      data-template-commit="960481d"
    >
      <aside className="side-rail">
        <Link
          className="archive-brand"
          href="/demo"
          aria-label="CaseFile 演示模式首页"
        >
        ...
        <nav className="archive-nav" aria-label="演示模式产品模块">
          {demoModules.map((module) => {
            const href = "href" in module ? module.href : undefined;
            const active = isDemoModuleActive(pathname, href);
            const className = `nav-row ${
              active ? "is-active" : href ? "is-ready" : "is-planned"
            }`;
            const content = (
              <>
                ...
                <span className="nav-state">
                  {active ? "当前" : module.status}
                </span>
              </>
            );
            return href ? (
              <Link className={className} href={href} key={module.no}>
                {content}
              </Link>
            ) : (
              <button
                ...
                onClick={() =>
                  setNotice(
                    `${module.label}属于后续阶段，本轮保留产品入口与职责说明。`,
                  )
                }
                type="button"
              >
        ...
        <div className="rail-footer">
          <button
            onClick={() =>
              setNotice("原型数据仅保存在当前浏览器 LocalStorage。")
            }
            type="button"
          >
            <span>□</span> 数据与安全
