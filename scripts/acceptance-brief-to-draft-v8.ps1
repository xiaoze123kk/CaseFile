...
.DESCRIPTION
v12-v14 默认执行 30 次并轮换五类时间、空间与竞争矩阵场景；v15 额外加入
2 假设 × 8+ 信息与 3 假设 × 8+ 信息两档密集竞争矩阵场景（共七类轮换），
...
param(
    ...
    [ValidateSet("brief-to-draft-v8", "brief-to-draft-v9", "brief-to-draft-v10", "brief-to-draft-v11", "brief-to-draft-v12", "brief-to-draft-v13", "brief-to-draft-v14", "brief-to-draft-v15", "brief-to-draft-v16", "brief-to-draft-v17", "brief-to-draft-v18")][string]$PromptVersion = "brief-to-draft-v15",
    [string]$ReportPath = "",
    [string]$ModelId = "",
    [string]$Scenarios = "",
...
$env:CASEFILE_LIVE_ACCEPTANCE_MODEL_ID = if (
    $Provider -eq "deepseek" -and [string]::IsNullOrWhiteSpace($ModelId)
) {
    "deepseek-flash"
} else {
    $ModelId
