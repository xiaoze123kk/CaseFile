...
$files = @(Get-ChildItem -LiteralPath $MigrationDirectory -File -Filter 'V*.py' | Sort-Object Name)
if ($files.Count -eq 0) {
    throw "No timestamp Alembic migrations found in $MigrationDirectory"
}

foreach ($file in $files) {
    $nameMatch = [regex]::Match($file.Name, $namePattern)
    if (-not $nameMatch.Success) {
        $errors.Add("Invalid migration name: $($file.Name). Expected VyyyyMMddHHmmss__lower_snake_case.py")
        continue
    }
...
for ($index = 0; $index -lt $orderedEntries.Count; $index++) {
    ...
    if ($entry.DownRevision -ne $expectedDownRevision) {
        $errors.Add(
            "Invalid chain at $($entry.File): down_revision=$($entry.DownRevision), expected $expectedDownRevision"
        )
    }
}
