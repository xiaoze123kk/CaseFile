...
| `fixtures/general_mutation_benchmark/` | General Mutation Capability Dev 与 Safety / Abstention：Capability 使用自然语言输入、最终状态 Oracle 与隔离 Reference Plan；Safety v2 冻结危险请求、隐式歧义和带 PatchOperation Oracle 的合法近邻，Reference/Oracle 均不进入 Provider 输入。 |
