...
| `backend/src/casefile/benchmark/policies/prose-rewrite-qualification-v1-descriptor.json` | B2 私有 qualification fail-closed descriptor：只公开 24 题分布、冻结门槛、public/private suite hash、author/reviewer attestation hash 和 review readiness；私有正文与 Gold 仅位于 `backend/var/benchmark/private/prose-rewrite/qualification-v1/`，不进入 Git。当前 `pending_independent_review / qualification_eligible=false`。 |
