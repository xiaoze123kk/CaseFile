...
| `contracts/schemas/compiler/scene-compiler.schema.json` | N4.4 v2 完整冻结输入、章内最多八场的 Provider-facing ModelView 与受控 SemanticFill 契约；ModelView schema v1 兼容 projection v1/v2/v3，v2 保证可见引用目录闭包，v3 以 `beat_basis_allowlist` 显式呈现顶层 Beat provenance 精确白名单；模型不得控制最终 ID、规划顺序、Exposure 或 Resolution 权威字段。 |
...
| `fixtures/scene_plan_benchmark/v1/` | N4.4 Narrative Execution Benchmark 审计源：冻结 8 能力×basic/decoy/dense 的 NovelPlan/NarrativeIR 输入、人工审阅 Reference、每能力一个合法 Alternative、11 个面向 v2 SemanticFill/State Engine 的确定性 Safety Mutation、精确 Pro 3-trial 资格参数与 contract-only G3 rubric；Runner 确定性绑定 N4.3 v4 Profile/Exposure 后生成 v2 Input/ModelView，资格在首轮 live baseline 后仍保持未晋级。 |
