...
| `contracts/schemas/compiler/prose-rendering.schema.json` | N4.5 Checklist、SceneRender、Judge/Consensus、旧 Quality 报告及 B3 v4 五维 Assessment candidate/report、授权窗口 Patch candidate、Quality Delta、NovelCandidate 与 CompileManifest 的严格协议；Evidence 使用单 block、Unicode 半开区间和逐字原文。 |
...
| `fixtures/prose_quality_benchmark/v4/` | B3 v4 公开 development：8 组语义均通过且直接 lineage 的匿名 render 对、16 份五维 severity/Evidence Gold、8 个确定性 Delta Gold、严格 suite/task/attestation hash 与零网络满分 runner；只用于开发，不构成资格。 |
...
| `backend/src/casefile/benchmark/policies/prose-quality-qualification-v4-descriptor.json` | B3 v4 私有资格 descriptor：冻结全新 16 组 Pointwise/Delta Holdout、4 组中性变体、24 个局部问题 Polisher 场景、Pro 模型、两个新 Prompt/component、3 窗/40% policy、目标维度 Delta policy、双 attestation 与相对 v1–v3 为零的题目指纹交集；正文与 Gold 仅位于忽略提交的 `qualification-v4/`。 |
