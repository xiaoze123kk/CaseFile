...
小说方案确认区提供“自动审编 / 快速初稿 / 完整精修”三档，默认自动审编。`novel-compiler-api.ts` 显式发送 prose_mode；记录区区分审编完成、有未解决问题和部分审核未完成，并可展开模型选稿理由。活动任务不允许修改选择，续跑不携带新模式；历史无 prose_mode 的记录按完整精修显示。首次完成统计不表示严格语义通过。
