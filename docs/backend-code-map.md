...
`backend/tests/integration/chat_feedback_browser_server.py` 是显式 opt-in 的隔离 FakeProvider HTTP/Worker 桌面验收入口；只接受指定反馈 `_test` 数据库，不连接开发库或真实 Provider，延迟注入只存在测试 Provider。

- `backend/src/casefile/agent_runtime/chat_preview.py`：有界顶层 answer 增量 JSON 解析、跨块公开语言检查、缓冲与预览失效；不持有数据库，不产生正式消息或修改事实。
- `backend/src/casefile/worker/chat_feedback.py`：独立 TaskRun/TaskAttempt/lease fencing 的短事务反馈写入，复用追加式 TaskEvent，执行每 Run 128 块/64KiB 上限；取消时静默停止发布，不新增安全点。
- `backend/src/casefile/application/chat_public_events.py`：保留 v1 投影；v2 重放真实活动身份、已确认对象引用、预览偏移及公开失效，不暴露原始 payload。公开文案失效后历史重放隐藏对应预览。
- `backend/src/casefile/application/workflow/goal_session.py`：新增只读投递列表及根据关系型 predecessor/source message 重建后继 Goal 关联。
- `backend/tests/unit/test_chat_preview.py`、`backend/tests/unit/test_chat_preview_transport.py`、`backend/tests/unit/test_chat_feedback_events.py` 与 `backend/tests/integration/test_chat_feedback.py`：解析、公开检查、严格工具流用量、版本兼容、重放、归属和 Attempt fencing 验证。

## 消息上下文快照

`backend/src/casefile/application/agent_message_context.py` 拥有规范化消息上下文写入、公共快照投影及 TaskRun 输入读取。`workflow/agent.py` 在发送事务中先冻结并裁剪引用，再原子写入用户消息、上下文和 TaskRun；`workflow/goal_session.py` 的 steer/replace/clarification continuation 从对应用户消息读取快照，不沿用上一条消息的选择。

`data_postgres/models/collaboration.py` 定义 `agent_message_contexts`（工作稿身份/revision/视图/消息归属）与 `agent_message_context_refs`（有序 typed stable ID 引用）。公共读取仍经过既有 Chat DTO 路由。`V20260903224420__agent_message_context_snapshot.py` 为历史 TaskRun 可靠输入回填；缺少 focus 或同消息存在不一致输入时不推测。
...
| `backend/src/casefile/domain/narrative_compiler/prose_quality.py` | N4.5-06 文学质量纯领域边界：验证语义接受前置、findings 精确 Evidence、原稿/润色稿直接 lineage、Preservation pass 与匿名双位置报告；只有两次整体均映射到 polished 且所有维度均不映射回 original 时才采用润色稿，其他组合确定性回滚。 |
...
| `backend/src/casefile/agent_runtime/prose_judge.py` | N4.5-02 Provider-neutral Semantic Council：冻结 Fidelity/Adversarial/Coherence 三策略消融、一次批量 Arbiter、无多数票 Consensus、单轮调用预算、精确 request fingerprint 恢复、原始响应审计以及 DeepSeek/Fake 适配；Fidelity v6 保持服务端 Evidence ID 与公共 Judge Report 组装边界，补齐必需事件时间、逐 check 独立、真实事件因果判定和 Evidence 必填项输出前自审；服务端仅把缺少必需 Evidence 的正向判定保守降为 `uncertain`，未知/重复 Evidence 等其他协议错误仍失败关闭；并仅对连接/超时执行一次同 fingerprint 的显式可审计重试，SDK 隐藏重试关闭。 |
...
| `backend/src/casefile/benchmark/prose_quality_eval.py` | N4.5 B3 Quality suite 严格 loader 与零网络 runner：公开 development 冻结 8 组 A/B 偏好、五维 Gold 和 2/4/2 的 A/B/Tie 分布，逐题执行匿名双位置比较并输出永不具备资格的脱敏报告；准确率只比较固定首位置与人工 Gold，交换位置只计算独立的镜像一致性，避免同一次位置偏差被双重处罚。私有 qualification loader 在 Provider 前校验 16 组 Holdout、24 个 Polisher Scene、双方 Semantic pass、直接 lineage、8 焦点分布、人工 Gold、Codex-owner 审阅 attestation 及全部内容/fingerprint/hash，正文只允许位于 Git 忽略目录。 |
| `backend/src/casefile/benchmark/prose_quality_qualification.py` | N4.5 B3 一次性 Live 资格执行器：在 clean/stable source 与不可复用 attempt manifest 上先执行 16×2 匿名 Quality Holdout，再执行 24 个 findings→Polisher→完整 Preservation Council→双位置 Pairwise；保持 16/24 固定分母，基础设施失败停止后续调用，按冻结 Quality/Polisher 门槛聚合，并只保存哈希、标签、判定、usage、latency 与脱敏 transport attempt。PowerShell 入口还会拒绝复用本机已有正式 attempt 的同版本 qualification package。 |
...
| `backend/tests/unit/test_prose_quality_critic.py` | 验证 N4.5-06 findings 的语义前置与精确 Evidence、Pairwise 来源匿名和相反位置、五维完整性、稳定采用/回滚规则、第二调用失败关闭以及 DeepSeek/Fake 脱敏审计。 |
| `backend/tests/unit/test_prose_quality_benchmark.py` | 验证 B3 公开 8 组 Quality 偏好对、双方 Semantic pass、Polisher 直接 lineage、五维 Gold、生成器确定性、双位置 Fake 满分、固定分母、哈希失败关闭及报告正文脱敏。 |
| `backend/tests/unit/test_prose_polisher.py` | 验证 Polisher 的 Semantic/findings 精确前置、完整候选规范化、直接 lineage、单调用与 exact recovery，以及完整 Preservation pass 后采用、语义失败跳过 Pairwise 并逐字回滚、tie/不稳定质量回滚的服务端闭环。 |
| `backend/tests/unit/test_prose_quality_qualification.py` | 验证 B3 一次性报告在 14/16、15/16、24/24、22/24、18/24 与精确回滚门槛边界的资格结论、固定分母、脱敏字段，以及非法/复用 attempt 在读取私有包和 Provider 前失败关闭。 |
...
小说编译的显式 `plan_execute` 开关冻结 `prose-shadow-runtime-v13`。运行时从已确认 ScenePlan 显式字段派生逐场及 setup/payoff 目标，Writer v7 与 Rewriter v10 返回正文和独立核对，逐场调用记录及最终对账/摘要写入不可变 AgentStepRun 日志。审核调用不增加 Nag 计数；未来范围不会催办；最终对账失败不改变正文原交付状态。生产默认仍为 runtime v12。
