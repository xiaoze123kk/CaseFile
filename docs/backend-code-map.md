...
| `backend/src/casefile/agent_runtime/chat_execution.py` | Worker 与 M2 共用的纯执行内核：对冻结 Chat Request 调用 Provider、执行完成前引用与 audit finding 证据校验、v15 Finalizer 后服务器补丁门禁、能力完整性 repair 和失败时至多一次定向修复，并合并 usage/tool metrics；不依赖 SQLAlchemy、FastAPI 或持久化。 |
