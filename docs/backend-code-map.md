...
| `backend/src/casefile/agent_runtime/context/policies/` | Policy-as-data 资源：`schema.json` 校验版本化 Context Policy 文档；`loader.py` 按 `context_policy_version` 从不可变 JSON 加载并校验策略；`agent-focus-v1` legacy 策略通过 `legacy_full_injection_v1` Stage 对现有全量注入输入只计量不删改；`casefile-chat-context-v1` 为阶段 2 正式策略：skeleton→focus_objects→history_window→validation_trim→chat_contract 五段装配。M0/M1 验收通过后已切为**默认策略**并配对 Prompt v4；`CASEFILE_CHAT_CONTEXT_ROLLOUT=agent-focus-v1` 可整组回退 legacy。 |
