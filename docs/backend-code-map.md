...
| `backend/src/casefile/domain/narrative_compiler/prose_quality.py`、`prose_quality_v2.py` | v1 保留 findings、镜像 Pairwise 与历史选择；v2 定义匿名五维 assessment、Evidence 派生的 3 窗/40% 编辑授权、窗口补丁确定性应用和 Quality Delta，只有五维无回退且原问题目标维度至少一项改善才采用润色稿。 |
...
| `backend/src/casefile/agent_runtime/prose_quality_assessor.py`、`prose_patch_polisher.py`、`prose_polish_supervisor_v2.py` | B3 v4 并行运行时：固定 Pro 模型执行匿名单稿五维评估与授权窗口补丁；Supervisor v2 提供 no-op、范围超限、空补丁、Preservation 和 Delta 的失败关闭/精确回滚路径，最大七次模型调用，不调用旧 Pairwise。 |
...
| `backend/src/casefile/benchmark/prose_quality_v4_eval.py`、`prose_quality_v4_qualification.py` | B3 v4 的公开 8 组/16 次匿名 assessment 与 Delta 开发 runner，以及私有 16 组/32 次 assessment、4 组中性变体和 24 个窗口 Polisher 的一次性 Live 执行器；冻结 Pro、Prompt/component、窗口/Delta policy、clean source、不可复用 attempt 与 hash-only 报告。 |
