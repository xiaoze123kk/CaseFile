...
| `backend/src/casefile/domain/narrative_compiler/prose_checklist.py` | N4.5 Profile v2 跨字段门禁、ScenePlanIR v2 状态重放、前一场 accepted Render 绑定、对象来源闭包与固定 policy 的逐 Scene Checklist 构造/复验；同时校验服务器 Render 身份、Unicode 字符数及 Judge 全量覆盖和逐字 Evidence，不执行自然语言语义判断。 |
...
| `backend/src/casefile/agent_runtime/prose_judge.py` | N4.5-02 Provider-neutral Semantic Council：冻结 Fidelity/Adversarial/Coherence 三策略消融、一次批量 Arbiter、无多数票 Consensus、单轮调用预算、精确 request fingerprint 恢复、原始响应审计以及 DeepSeek/Fake 适配；v4 请求携带带稳定 ID 的服务端 Evidence 目录，模型只返回逐 check verdict/rationale/Evidence ID 私有候选，Runtime 拥有 Scene/Checklist/Render identity、Unicode Evidence 展开与公共 Judge Report 组装复验；协议失败不重试，基础设施失败使 attempt inconclusive。 |
...
| `backend/src/casefile/benchmark/prose_judge_eval.py` | N4.5-02 公开 B0 开发集严格 loader、Fake/Mutation 门禁和三策略 DeepSeek 消融；N4.5-02R 提供仅 Fidelity 的固定三样本 Smoke，以及依次验证 Fidelity、Adversarial、Coherence 和一次 Gold-backed 强制争议 Arbiter 的固定四调用 Council Smoke，两者永不具备资格并逐项验证 Prompt/服务器 hash/Evidence 目录/报告覆盖/原始调用审计；完整消融校验 8×3 分布、72 份唯一正文、Gold Evidence、双遍开发 attestation 与全部内容哈希，按冻结门槛选择每任务中位请求数最低的合格策略，并生成不含模型正文的开发报告与 `qualified=false` policy descriptor。 |
