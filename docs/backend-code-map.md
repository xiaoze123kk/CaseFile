...
| `backend/src/casefile/benchmark/prose_quality_eval.py` | N4.5 B3 公开 Quality development suite 严格 loader 与零网络 runner：冻结 8 组 A/B 偏好、五维 Gold 和 2/4/2 的 A/B/Tie 分布，逐题证明双方 Semantic pass、直接 Polisher lineage、内容哈希与开发 attestation；每题执行匿名双位置比较，聚合 overall、position consistency、40 个维度标签及固定失败分母，只输出脱敏且永不具备资格的报告。 |
