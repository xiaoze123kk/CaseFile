...
| `backend/src/casefile/benchmark/prose_judge_eval.py` | N4.5-02 公开 B0 开发集严格 loader、Fake/Mutation 门禁和三策略 DeepSeek 消融；N4.5-02R 另提供固定 explicit/implicit/adversarial 三样本、仅 Fidelity、最多三调用且永不具备资格的 Provider 协议 Smoke，逐项验证服务器 hash 复制、报告覆盖、Evidence 和原始调用审计；完整消融校验 8×3 分布、72 份唯一正文、Gold Evidence、双遍开发 attestation 与全部内容哈希，按冻结门槛选择每任务中位请求数最低的合格策略，并生成不含模型正文的开发报告与 `qualified=false` policy descriptor。 |
