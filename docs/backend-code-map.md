...
| `backend/src/casefile/domain/narrative_compiler/scene_fill.py`、`scene_state_engine.py` | N4.4 模型字段纯领域门禁与 ScenePlanIR v2 权威状态引擎：Scene Fill 拒绝保留 batch/scene/beat/path/ref/allowlist hash 安全证据；State Engine 重建 Exposure，应用知识/地点增量，规范化因果和 setup/payoff，并通过完整重放 Linter 失败关闭。 |
...
| `backend/src/casefile/agent_runtime/scene_compiler.py`、`scene_compiler_prompt.py` | N4.4 影子 Scene Fill 编排：章内最多八场、逐批状态哈希链、exact-hash 恢复、单轮无工具 Prompt 与模型所有字段的确定性越权门禁；Prompt v2 明确区分对象目录存在性与 `beat_basis_allowlist` 顶层 provenance 权限，Validator 语义保持不变。 |
...
| `backend/src/casefile/benchmark/scene_plan_eval.py` | N4.4 ScenePlan v2 独立评测：把审计后的 8×3 capability fixtures 升级为完整 v2 Input/ModelView，复用正式 Shadow Scene Fill runtime、Provider adapter 与 State Engine；v2.2 Runner 支持 Fake regression、v2 Safety mutations、DeepSeek/OpenAI live capability、精确 Pro 3-trial、严格 checkpoint/resume、失败 batch ordinal、可选 failed-proposal 诊断、G0–G2 outcome、contract-only G3、G4 结构语义签名与未晋级 baseline 报告。 |
