...
| `backend/src/casefile/domain/narrative_compiler/planner_input.py`、`planner_constraints.py`、`novel_plan.py` | N4.3 纯领域 PlannerInput v1/v2 冻结、确定性 PlannerView 与独立 PlannerConstraintIR 投影/复验，以及 NovelPlanCandidate 引用/披露/Resolution/时序/DAG/Profile 门禁、结构化语义 counterexample、候选保持型 presentation mode 有限修复、服务器 SourceRef 派生、规范索引与稳定 NovelPlanIR hash；修复只改叙述模式且不得减少原有 flashback/flashforward，ConstraintIR 不自动排程。 |
...
| `backend/src/casefile/worker/executors/story_planner.py` | N4.3 Planner 编排：完整 fingerprint 复用、成功 ModelCall 原文重放、候选保持型确定性时序修复审计、stale call 诊断，以及 Step/Event/NovelPlan Artifact 同事务提交；修复无解时仍由权威 Validator 失败关闭。 |
| `backend/src/casefile/benchmark/novel_plan_eval.py` | Novel Plan Regression/Safety/24 Task Capability Benchmark、v1/v2 输入单变量选择、G2 oracle evidence 审计、生产/G2 失败分层、动态 cohort、严格 fingerprint checkpoint/resume、G0–G3 分层与精确 Pro baseline/候选晋级门禁。 |
