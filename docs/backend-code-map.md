...
| `backend/src/casefile/agent_runtime/scene_compiler.py`、`scene_compiler_prompt.py` | N4.4 影子 Scene Fill 编排：章内最多八场、逐批 typed state/hash 链、exact-hash 恢复、单轮无工具 Prompt 与模型所有字段的确定性越权门禁；Prompt v2 冻结对象目录与 `beat_basis_allowlist` 权限分层，Prompt v3 进一步锁定 obligation.kind 与 Beat.kind 一致且禁止跨 kind 合并；Validator 语义保持不变。 |
