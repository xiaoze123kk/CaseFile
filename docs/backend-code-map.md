...
| `backend/src/casefile/agent_runtime/chat_execution.py` | Worker 与 M2 共用的纯执行内核：对冻结 Chat Request 调用 Provider、执行完成前引用与 audit finding 证据校验、通用 suggestion 字段和值门禁、路由 suggestion 权限收束、v15 Finalizer 后服务器补丁门禁、能力完整性 repair 和 usage/tool metrics 合并；先进行一次最小定向修复，只有 v15 逻辑审计中服务器能证明单一 `object_id/path/finding_ref` 缺口时才允许第二级 target-locked repair，第三次仍失败立即 fail-closed；不依赖 SQLAlchemy、FastAPI 或持久化。 |
