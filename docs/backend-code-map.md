...
| `backend/src/casefile/agent_runtime/prose_polisher.py` | N4.5-06 Provider-neutral Polisher：固定 `deepseek-v4-pro` 单轮无重试输出完整 Scene candidate；Provider 前证明原稿 Semantic pass 与 findings 精确绑定，冻结 Prompt/Schema/input/component fingerprint，服务端规范化为与原稿直接相连、同 round 的 `polished` Render，并提供 DeepSeek/Fake、exact-hash 恢复与脱敏审计。v2 将 Profile 字符范围作为模型质量指导，不再由服务端仅因偏离目标范围拒绝润色稿；Schema、身份/lineage、hash、Evidence、预算与完整 Preservation Council 仍失败关闭。 |
