...
| `backend/src/casefile/domain/logical_mutation/repair/` | M3.3 纯领域修复内核：版本化 RepairPolicy、角色化 ClosureObligation、MutationSimulation 资格评估，以及由合格 assessment 确定性生成的 RepairScopeV1/ClosureRepairContextV2；V2 在原冻结上下文上增加纳入 hash 的服务端 `allowed_writes/value_schema`，V1 类型仅用于历史回放。`RepairProposer` 是 Provider-neutral 端口；Repair Engine 仅接受 scope 内 UPDATE，最多两轮从同一 baseline 重放原 mutation 与折叠后的 companion updates，以 obligation 消失、blocking 不增加、candidate hash 去重执行 no-progress/oscillation 门禁。最终通过 top-level semantic document diff 重建 MutationSet，排除可由 Normalizer 再生的 mechanical projection，并以 document/hash/full closure 二次证明。未知规则、hard、混合 manual、intent revision、非法 proposal、预算超限、stale binding 与 rebase mismatch 全部失败关闭；本模块不接 Provider/数据库/API/UI，不执行 Apply。 |
...
| 路径 | 职责 |
|---|---|
| `backend/src/casefile/benchmark/` | `brief_to_draft` Provider 级 Fixture 运行器与指标汇总；记录 CaseFile 结构有效率、模型调用/工具协议、修复次数、延迟和结构化诊断覆盖率。它明确不验证 TaskRun、Worker、持久化、SSE 或候选采用边界，不能单独作为发布验收。`closure_repair_eval.py` 保持 24 场景 Regression/Safety 默认入口和 CI 零容忍门禁，并分派显式 Capability CLI；`eval_core.py` 定义 benchmark-local Suite/Task/Trial/Transcript/Outcome/Grader/Report 类型；`closure_repair_capability.py` 严格加载 61 个 input/oracle 分离 Task，校验全部 Repair Policy 与 Reference，12 个 agent Task 复用 Simulation → Assessment → Scope → Context → DeepSeek → Engine → Rebase Proof，49 个 manual/ineligible Task 复用 Policy Decision Kernel 并禁止 Provider 调用，分开汇总能力、正确拒绝、安全、效率、基础设施失败、协议修复与条件二轮恢复，记录 Agent contract/Prompt provenance，并支持 v1/v2 受控实验比较和逐 Trial artifact。`chat_context_eval.py` 运行 casefile-chat 上下文策略基线：五类冻结样本经 `build_chat_context_manifest` 计量后生成 `var/benchmark/context-baseline-v1.json`，并实现阶段 3 Boundary Continuation Eval——同一 Transcript 断点分别用完整原文与压缩后 Thread Memory 继续，确定性比较 Task Success/State Recall/Action Continuity/Repeated Work/Peak & Total Tokens 六道门禁，报告写入 `var/benchmark/context-boundary-v1.json`；`--gate-boundary` 供 `scripts/check.ps1` 作 M0 门禁。`context_tier_benchmark.py` 运行阶段 4 四档策略 A/B 并执行无回退和预算门禁；`validator_eval.py` 运行不调用 Provider 的 V0/V1/V2 确定性 release gate。 |
...
| `backend/src/casefile/agent_runtime/closure_repair.py`、`closure_repair_prompt.py` | M3.3 Provider-facing Closure Repair 契约与 Prompt Package 渲染：只接受冻结 Context/round，严格输出 obligation/object/path/value/reason，并转换为 Provider-neutral `RepairProposal`；不拥有 Scope、Simulation、Rebase Proof、债务授权或 Apply 决策。 |
