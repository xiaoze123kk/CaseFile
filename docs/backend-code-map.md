...
| `backend/src/casefile/domain/narrative_compiler/prose_checklist.py` | N4.5 Profile v2 跨字段门禁、ScenePlanIR v2 状态重放、前一场 accepted Render 绑定、对象来源闭包与固定 policy 的逐 Scene Checklist 构造/复验；同时把严格 SceneRenderCandidate 规范化为服务端拥有身份的 Writer Render，并校验 Unicode 字符数、Judge 全量覆盖和逐字 Evidence，不执行自然语言语义判断。 |
