...
| `backend/src/casefile/domain/narrative_compiler/scene_compiler_input.py` | N4.4 v2 冻结输入与最小模型视图投影：绑定 NovelPlan/NarrativeIR/Exposure/Profile，派生硬执行义务和来源状态种子，按章生成最多八场的稳定批次，并保证 Scene/State 中每个 Provider 可见 ObjectRef 都在同批对象目录闭包内。 |
