...
| `backend/tests/unit/test_prose_rewriter.py` | 验证 N4.5-05 完整 Scene Rewrite 请求、失败/保持语义投影、直接 Render lineage、精确恢复、DeepSeek/Fake 脱敏审计，以及初稿通过、Round 1 rescue、Round 2 封顶、预算和失败分类。 |
| `backend/tests/unit/test_prose_rewrite_benchmark.py` | 验证 B2 24 题矩阵、坏正文/Consensus/previous-pass/hash 重建、生成器一致性、两轮 Fake rescue 指标、固定分母、报告脱敏，以及 private qualification descriptor 自哈希和独立复审前 fail-closed。 |
