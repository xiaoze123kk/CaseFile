...
General Mutation、PatchSet 和 `V1EditingService` 继续拥有唯一写入权威。Patch 拒绝进入 clarification，Draft 变化进入 stale 且不自动 rebase，Apply 后的新 Draft revision 作为新 baseline；任何 Goal 自动 Apply、跨 Project、多 Agent、无限循环或模型决定 Observation 复用均被禁止。M3.8-03 至 06 已实现无数据库 Checkpoint、三类 cooperative safe point、HITL/Patch Review、语义 Amendment、replace/follow-up、read-only capability observation 持久化，以及带 TaskAttempt fencing 的 delivery claim/过期恢复和“旧切片终态 + 单条 control 消费 + 新 Revision/Goal + 唯一 continuation TaskRun”原子事务。M3.8-07 的 Interactive Goal qualification 只在 benchmark composition root 注入默认关闭的 `GoalSafePointObserver`，不把内部 safe point 暴露为 HTTP、DTO、事件或环境变量；正式资格结论必须来自 clean revision 上完整 24×3 Pro Holdout，定向测试不能代替。
