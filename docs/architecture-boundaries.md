...
General Mutation、PatchSet 和 `V1EditingService` 继续拥有唯一写入权威。Patch 拒绝进入 clarification，Draft 变化进入 stale 且不自动 rebase，Apply 后的新 Draft revision 作为新 baseline；任何 Goal 自动 Apply、跨 Project、多 Agent、无限循环或模型决定 Observation 复用均被禁止。M3.8-03 已实现无数据库 Checkpoint、三类 cooperative safe point、read-only capability observation 持久化，以及“旧切片终态 + 单条 steer 消费 + 新 Revision + 唯一 continuation TaskRun”的原子事务；Patch Review、语义 Amendment、replace/follow-up 和恢复 lease 仍属于后续阶段。
