...
| `apps/web/features/analyst-workbench/analyst-workbench.tsx`、`analyst-workbench.module.css`、`prototype-relay.module.css`、`analyst-fixture.ts` | `/demo` 单页分析师工作台的种子驱动交互、石墨纸＋档案铜视觉与确定性 Fixture；案名、对象、事件、问题、来源、关系、推理路径、卷宗、抽屉与审计随候选变化，同时保留五区布局（左侧对象目录经分隔条可拖拽调整宽度）、多视图同步、补丁审批、命令面板和响应式分区；主画布关系图与推理图均为铺满画布的可拖动节点图（关系图保留 fixture 初始坐标、推理图自动按路径分列布局，节点拖动时边实时跟随，拖动超过阈值不误触点击联动）；推理图按支持/竞争/已排除三种收束边与图例表达，证据与结论节点点击联动对象目录，画布下方提供逐路径推理表与关系表文字替代；接力条只表达预览稿、当前工作稿和旧简报状态，采用按钮经 Provider 接真实候选采用。 |
