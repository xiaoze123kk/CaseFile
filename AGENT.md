...
- 仓库采用前后端同仓、运行时分离的模块化单体。当前实现包含 PostgreSQL 数据底座、目标无关的 CaseFile v1 唯一机器契约、真实 SourceRecord → 可恢复 Brief Intake 追问/候选 → Brief 审阅/原子确认 → Agent → Validator → 不可变 Draft 候选 → 人工采用 → Snapshot 纵向切片；同一 BriefVersion 可重复生成候选，但项目始终只有一个当前可编辑 Draft。系统还包含基于最新完整 CaseFile 的持久多线程 Agent 对话、结构化建议逐项审阅、整批应用/撤销和过期门禁。`apps/web` 的正式开发基线只有 `/` 五阶段建案中心和 `/workbench` 分析师工作台：建案、Agent 任务、Brief、候选和采用走真实后端；工作台画布仍使用确定性 fixture，仅通过接力条承接真实候选并执行真实采用。两页共用 `ProductShell` 下的内存 `CaseSessionProvider`，刷新或 Provider 重挂载即开启空白新会话，不使用 LocalStorage；本地开发身份固定为 actor 1。旧 `/demo/intake`、`/brief` 非永久跳转到 `/`，其余 `/demo`、`/demo/*`、`/reasoning`、`/quality` 非永久跳转到 `/workbench`，正式导航不得产生 `/demo` URL。Validator 当前覆盖结构、引用和确定性语义门禁；Simulation、Compiler 和任何 Target Adapter 后端仍未实现。
...
| `backend/src/casefile/data_postgres/repositories.py` | 按 Project/Draft/Snapshot 聚合封装所有者过滤、对象注册读取、Operation 追加和 Snapshot 持久化。 |
...
| `backend/src/casefile/application/commands.py` | 与 HTTP 解耦的 Project 类型化写入命令。 |
...
| `backend/src/casefile/application/services.py` | Project 生命周期、Draft 契约投影和 Snapshot 的事务边界、并发控制及应用规则；对象编辑统一由 `v1_editing.py` 承担。 |
...
| `backend/src/casefile/api/app.py` | 应用工厂、启动数据库门禁、统一错误体、健康检查、Project/Draft/Snapshot 和通用对象 Patch 路由；不再公开旧 Entity/Event 专用 CRUD。 |
...
| `backend/tests/integration/test_api_vertical_slice.py` | 在真实 `_test` PostgreSQL 验证 Provider 设置保存/删除、原稿/润色候选、Brief 原子确认、三类 TaskRun、Draft 候选查询/采用、SSE 恢复、完成门禁和已删除 Entity/Event 路由的 404 边界。 |
...
| `apps/web/app/layout.tsx`、`app/providers.tsx`、`app/globals.css` | App Router 根布局、React Query Provider、单一产品壳挂载点和两个正式页面共用的最小全局样式。 |
| `apps/web/app/page.tsx`、`app/workbench/page.tsx` | `/` 五阶段建案中心与 `/workbench` 分析师工作台的正式路由壳；兼容跳转只在 `next.config.ts` 定义。 |
| `apps/web/components/product-shell.tsx`、`components/settings-entry.module.css` | 两页共用产品壳，在共同祖先挂载 `CaseSessionProvider` 支持不刷新的候选接力，并提供模型服务设置入口。 |
| `apps/web/public/casefile-brand.png` | 用户确认并按导航栏尺寸优化的 CaseFile 品牌位图；由两个正式页面的品牌容器展示。 |
| `apps/web/public/intake-pencil-dossier.svg` | 建案中心的低对比铅笔卷宗底纹；只承担背景氛围，不承载可交互信息。 |
...
| `apps/web/lib/local-session.ts` | 当前个人本地开发环境的唯一 actor 常量；不得扩展为浏览器身份存储。 |
| `apps/web/tests/` | 正式路由、兼容跳转、五阶段建案、跨页接力、真实采用、fixture 工作台、设置弹窗和无浏览器存储边界测试。 |
| `apps/web/tests/analyst-workbench.test.tsx` | 验证三类候选种子、事件联动、问题证据对照、Agent 补丁人工批准、来源抽屉、命令面板和切换种子后的局部状态重置。 |
| `apps/web/tests/case-session-state.test.tsx`、`intake-center.test.tsx` | 验证纯模型引用、冻结/修订/旧稿门禁、Provider 重挂载重置，以及 mock 真实 API 下的五阶段建案、候选预览/采用和状态隔离。 |
...
| 路径 | 职责 |
|---|---|
| `contracts/schemas/` | 面向跨语言消费者的 CaseFile、Brief、Brief Intake、编辑、验证与任务 Schema 集合。 |
