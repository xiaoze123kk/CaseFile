...
| `apps/web/features/analyst-workbench/analyst-workbench.tsx`、`analyst-workbench.module.css`、`prototype-relay.module.css`、`analyst-fixture.ts` | `/demo` 单页分析师工作台的种子驱动交互、石墨纸＋档案铜视觉与确定性 Fixture；案名、对象、事件、问题、来源、关系、推理路径、卷宗、抽屉与审计随候选变化，同时保留五区布局、多视图同步、补丁审批、命令面板和响应式分区；主画布推理图按“证据 → 铜脊线推理步骤 → 假设收束”三层展示支持/竞争/已排除路径并提供推理表文字替代，证据与假设按钮联动对象目录；接力条只表达预览稿、当前工作稿和旧简报状态，采用按钮经 Provider 接真实候选采用。 |
