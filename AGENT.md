...
| `scripts/bootstrap.ps1` | 幂等准备本地 `.env` 与加密主密钥、两个 PostgreSQL 容器、开发库迁移、42 表/head 验证，以及可选开发用户种子。 |
...
| `backend/src/casefile/data_postgres/models/workflow.py` | `briefs`、不可变 `brief_versions`、不可变 `source_records`、四类 `task_runs`、`task_attempts` 与不可变 `task_events` ORM；`casefile_chat` Run 通过复合外键绑定 Thread 及输入/输出消息。 |
...
| `backend/src/casefile/application/casefile_v1.py` | 在目标无关的 v1 CaseFile JSON 与规范化当前态之间执行原子写入、完整投影、契约引用映射和规范哈希，并向类型化编辑服务公开稳定的 ObjectRef 遍历；旧物理 Phase/公平性字段不再进入 v1 投影。 |
...
| `backend/src/casefile/application/workflow_service.py` | Provider 设置、不可变 SourceRecord、Brief 草稿/原子确认/冻结版本、四类 TaskRun，以及 Agent Thread/Message、建议逐项决策、整批应用/撤销、过期检测、最近任务恢复与 SSE 事件查询的事务边界。 |
| `backend/src/casefile/api/schemas.py` | FastAPI 严格请求 DTO 与应用命令转换，包含 Thread 创建/更新、Message 发送和 PatchSet 应用/撤销请求。 |
...
| `backend/src/casefile/api/workflow.py` | Provider、SourceRecord、Brief、四类 TaskRun、Agent Thread/Message、PatchSet 应用/撤销、最近任务恢复、TaskEvent/SSE、v1 CaseFile 读取和类型化编辑的 HTTP 路由。 |
...
| `backend/src/casefile/worker/` | 基于 PostgreSQL `FOR UPDATE SKIP LOCKED` 的四类 TaskRun 领取、lease/Attempt 恢复、Agent 执行、结果/事件原子持久化；`brief_to_draft` 执行结构修复、Validator 和 Snapshot 落库，`casefile_chat` 完成助手消息并保存可审阅 PatchSet。 |
...
| `backend/tests/integration/test_foundation_migrations.py` | 在明确的可丢弃 PostgreSQL `_test` 库验证八段升降级、42 表、SourceRecord/Agent 协作/注册/子类型门禁、引用、归属、并发、Canon 门禁和不可变触发器。 |
...
| `apps/web/features/workflow/` | 原稿建案、独立润色候选审阅、Brief 原子拆解/人工确认、Brief → Draft 和工作台的唯一产品实现，以及设置弹窗、SSE 可恢复安全审计轨迹与完成门禁；直接接真实 API/Workflow Store，不得另建 Fixture 版页面。 |
