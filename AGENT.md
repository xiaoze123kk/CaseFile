...
- 仓库采用前后端同仓、运行时分离的模块化单体。当前实现包含 PostgreSQL 数据底座、目标无关的 CaseFile v1 唯一机器契约、真实 SourceRecord → 可恢复 Brief Intake 追问/候选 → Brief 审阅/原子确认 → Agent → Validator → 不可变 Draft 候选 → 人工采用 → Snapshot 纵向切片；同一 BriefVersion 可重复生成候选，但项目始终只有一个当前可编辑 Draft。系统还包含基于最新完整 CaseFile 的持久多线程 Agent 对话、结构化建议逐项审阅、整批应用/撤销和过期门禁；`apps/web` 使用 React/Next.js 实现真实产品壳与“数字档案纸”界面。为便于逐页对照，`/demo`、`/demo/brief`、`/demo/workbench`、`/demo/reasoning`、`/demo/quality` 独立承载 `960481d` 前端原型基线；它只使用隔离的 Prototype Store/LocalStorage，不得请求真实 API、写入数据库或混用真实 Workflow 状态。该基线是冻结的对照模板，不是第二套生产实现。Validator 当前覆盖结构、引用和确定性语义门禁，Simulation、Compiler 和任何 Target Adapter 后端仍未实现。
...
| `backend/src/casefile/agent_runtime/` | 目标无关的 Prompt 输入渲染、OpenAI Responses/DeepSeek Chat Completions/Fake Provider、AES-256-GCM 用户密钥，以及 `brief_polish`、`brief_anchor_extract`、`brief_intake_questions`、`brief_intake_synthesize`、`brief_to_draft`、`casefile_chat` 的结构化结果、工具注册与 Validator 指标；不输出模型内部思维文本。 |
...
| `backend/src/casefile/agent_runtime/prompts/` | 六个 Agent 功能各自独立、不可变的版本化 System Prompt 唯一 Git 事实源；`registry.json` 只维护当前版本指针，版本目录同时保存 Manifest、原文和发布规则。 |
...
| `apps/web/app/layout.tsx`、`app/providers.tsx`、`app/globals.css` | App Router 根布局、真实 Workflow Provider、演示 Prototype Provider、真实/演示模式各自设计令牌和“数字档案纸”全局基础样式。 |
| `apps/web/app/page.tsx`、`app/brief/page.tsx`、`app/workbench/page.tsx` | 真实原稿建案/Agent 润色审阅、Brief 原子确认/生成和 v1 工作台路由壳。 |
| `apps/web/app/demo/` | `960481d` 前端原型基线的独立演示路由壳；所有页面只使用 Prototype Store，不得导入真实 Workflow/API。 |
| `apps/web/components/app-shell.tsx`、`components/archive-shell.tsx`、`components/demo-archive-shell.tsx` | 按路由选择真实或演示产品壳；真实壳负责项目、Provider 和工作流导航，演示壳冻结原型导航、命令面板、重置入口及返回真实模式入口。 |
| `apps/web/public/casefile-brand.png` | 用户确认并按导航栏尺寸优化的 CaseFile 品牌位图；由前端品牌容器居中裁切展示。 |
| `apps/web/public/intake-pencil-dossier.svg` | A 路径建案工作区的低对比铅笔卷宗底纹；复刻评审图中的叠纸、回形针、证据夹、印章与红色线索路径，只承担背景氛围，不承载可交互信息。 |
| `apps/web/components/archive-ui.tsx` | 无 Store 依赖的 Case Spine、文档头、面板头、状态徽记等全站设计系统组件。 |
| `apps/web/components/prototype-ui.tsx`、`apps/web/features/intake/`、`apps/web/features/workbench/` | 从 `960481d` 恢复的演示模板视觉组件、建案/Brief 和原型工作台；只允许为 `/demo` 路由前缀和模式切换做兼容调整。 |
| `apps/web/features/reasoning/` | `960481d` 演示模板中的整卷生成态、路径总览、React Flow 因果画布和候选审阅器；仅由 `/demo/reasoning` 使用。 |
| `apps/web/features/quality/` | 演示模板中的 ValidationIssue 筛选、确定性证据链、PatchCandidate 人工审阅和显式重新验证。 |
| `apps/web/features/benchmark/` | Benchmark 配置、运行与结果界面。 |
| `apps/web/features/simulation/` | 玩家模拟配置、运行进度和报告。 |
| `apps/web/features/compiler/` | 演示模板中的固定快照编译门禁、目标配置、产物清单和本地构建状态。 |
...
| `apps/web/features/workflow/intake-model.ts`、`intake-source-badge.tsx`、`brief-intake-workspace.module.css` | Intake 的纯展示模型、四类来源标识和数字档案纸/铅笔底纹布局；不请求 API、不持久化服务端事实。 |
...
| `apps/web/tests/intake-idea-step.test.tsx` | 验证最初想法页在 Agent 润色时展开为原文只读、候选可编辑的左右内联对校，且不退回遮罩弹窗。 |
