...
| `apps/web/features/analyst-workbench/analyst-workbench.tsx`、`analyst-workbench.module.css`、`prototype-relay.module.css`、`analyst-fixture.ts` | `/demo` 单页分析师工作台的种子驱动交互、石墨纸＋档案铜视觉与确定性 Fixture；案名、对象、事件、问题、来源、关系、推理路径、卷宗、抽屉与审计随候选变化，同时保留五区布局、多视图同步、补丁审批、命令面板和响应式分区；主画布推理图把全部推理路径合并为一张铺满画布的可拖动推理图（结论收束顶部、步骤按路径分列、共享证据铺底，边随节点实时跟随），支持/竞争/已排除三种收束边与图例，证据与结论节点点击联动对象目录，画布下方提供逐路径推理表文字替代；接力条只表达预览稿、当前工作稿和旧简报状态，采用按钮经 Provider 接真实候选采用。 |
