...
- 仓库采用前后端同仓、运行时分离的模块化单体。当前实现包含 PostgreSQL 数据底座、目标无关的 CaseFile v1 唯一机器契约、真实 SourceRecord → Brief 审阅/原子确认 → Agent → Validator → 不可变候选 → 人工采用 → Snapshot 纵向切片；同一 BriefVersion 可重复生成候选，但项目始终只有一个当前可编辑 Draft。系统还包含基于最新完整 CaseFile 的持久多线程 Agent 对话、结构化建议逐项审阅、整批应用/撤销和过期门禁；`apps/web` 使用 React/Next.js 实现真实产品壳与“数字档案纸”界面。为便于逐页对照，`/demo`、`/demo/brief`、`/demo/workbench`、`/demo/reasoning`、`/demo/quality` 独立承载 `960481d` 前端原型基线；它只使用隔离的 Prototype Store/LocalStorage，不得请求真实 API、写入数据库或混用真实 Workflow 状态。该基线是冻结的对照模板，不是第二套生产实现。Validator 当前覆盖结构、引用和确定性语义门禁，Simulation、Compiler 和任何 Target Adapter 后端仍未实现。
...
| `backend/src/casefile/api/workflow.py` | Provider、SourceRecord、Brief、四类 TaskRun、Draft 候选列表/详情/采用、Agent Thread/Message、PatchSet 应用/撤销、最近任务恢复、TaskEvent/SSE、v1 CaseFile 读取和类型化编辑的 HTTP 路由。 |
