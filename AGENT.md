...
- 仓库采用前后端同仓、运行时分离的模块化单体。当前实现包含 PostgreSQL 数据底座、CaseFile v1 唯一机器契约、真实 Brief → 单 Agent → Validator → Snapshot 纵向切片，以及 `apps/web` 中 React/Next.js 实现的真实工作流和完全隔离的 `/demo/*` 原型。Validator 当前覆盖结构、引用和确定性语义门禁；Simulation 与 Compiler 后端仍未实现。
...
| `scripts/bootstrap.ps1` | 幂等准备本地 `.env` 与加密主密钥、两个 PostgreSQL 容器、开发库迁移、37 表/head 验证，以及可选开发用户种子。 |
...
| `backend/src/casefile/data_postgres/models/content.py` | Narrative Phase、Entity/Person、v1 Relationship/Location、Event、Information Unit/Evidence/Testimony、Claim 与 Knowledge State ORM。 |
...
| `backend/src/casefile/data_postgres/models/workflow.py` | `briefs`、不可变 `brief_versions`、`task_runs`、`task_attempts` 与不可变 `task_events` ORM。 |
...
| `backend/src/casefile/application/casefile_v1.py` | 在 v1 CaseFile JSON 与 37 表规范化当前态之间执行原子写入、完整投影、契约引用映射和规范哈希。 |
| `backend/src/casefile/application/v1_editing.py` | Entity、Location、Event 的有限字段编辑、revision 冲突检查和 v1 契约往返门禁。 |
| `backend/src/casefile/application/workflow_service.py` | Provider 设置、Brief 草稿/冻结版本、TaskRun 创建与 SSE 事件查询的事务边界。 |
...
| `backend/src/casefile/api/workflow.py` | Provider、Brief、TaskRun、TaskEvent/SSE、v1 CaseFile 读取和有限编辑的 HTTP 路由。 |
...
| `backend/src/casefile/agent_runtime/` | 单 Agent 版本化 Prompt、Agents SDK Provider、Fake Provider、AES-256-GCM 用户密钥、结构化结果、工具注册与 Validator 指标。 |
| `backend/src/casefile/worker/` | 基于 PostgreSQL `FOR UPDATE SKIP LOCKED` 的 TaskRun 领取、lease/Attempt 恢复、Agent 执行、结构修复、事件追加和 Snapshot 原子落库。 |
...
| `backend/tests/unit/test_foundation_metadata.py` | 静态验证精确 37 表、Identity 主键、JSONB 白名单、个人归属、文档同步和关键约束，不连接数据库。 |
...
| `backend/tests/integration/test_foundation_migrations.py` | 在明确的可丢弃 PostgreSQL `_test` 库验证六段升降级、37 表、注册/子类型门禁、引用、归属、并发、Canon 门禁和不可变触发器。 |
| `backend/tests/integration/test_application_services.py` | 在真实 `_test` PostgreSQL 验证生成 Worker、lease 恢复、不可变历史和 v1 有限编辑闭环。 |
| `backend/tests/integration/test_api_vertical_slice.py` | 在真实 `_test` PostgreSQL 验证 Provider 设置、Brief 冻结、TaskRun、SSE 与完成门禁闭环。 |

当前正式业务表恰好为 37 张：

- 身份、输入与任务：`users`、`projects`、`user_provider_settings`、`briefs`、`brief_versions`、`task_runs`、`task_attempts`、`task_events`。
...
| `apps/web/app/page.tsx`、`app/brief/page.tsx`、`app/workbench/page.tsx` | 真实五字段建案、Brief 冻结和 v1 工作台路由壳。 |
| `apps/web/app/demo/` | 完全隔离的旧原型建案、Brief、工作台、推理和质量路由；只使用 Prototype Store。 |
| `apps/web/components/real-archive-shell.tsx` | 真实模式侧栏、设置入口、工作流导航和显式演示入口。 |
| `apps/web/components/archive-shell.tsx` | 演示模式侧栏、原型模块导航、命令面板、本地重置和显式返回真实模式入口。 |
...
| `apps/web/features/workflow/` | 真实建案、Brief、设置弹窗、SSE 安全审计轨迹、完成门禁、12 类对象展示与三类对象有限编辑。 |
...
| `apps/web/store/workflow-store.tsx` | 真实工作流最小会话指针和前端恢复状态；服务端事实始终由 API 重新读取。 |
