...
| `apps/web/features/demo-prototype/demo-prototype-provider.tsx` | 演示路由共享的 reducer/Context 与会话后端适配；保存建案步骤、简报审阅版本、候选生成、预览与显式采用状态，计算 `pending/current/stale`，Provider 重挂载时恢复空白初态；润色/追问/成案/审阅/候选全流程通过 `demo-intake-api` 请求真实后端，不得导入 workflow-store 或使用浏览器存储。 |
...
| `apps/web/features/demo-prototype/demo-intake-mapping.ts` | 服务端 Brief Intake/Brief/Draft 候选 DTO 与原型模型（`PrototypeBrief`/问题/答案/审阅/候选）之间的纯函数映射；不请求 API。 |
