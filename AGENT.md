...
- 仓库采用前后端同仓、运行时分离的模块化单体。当前实现包含 PostgreSQL 数据底座、CaseFile 0.1.0 机器契约，以及项目、人物/地点、事件、语义引用和 Snapshot 的首个 FastAPI 纵向切片；`apps/web`、跨语言契约、Benchmark、Agent Runtime、Validator、Simulation、Compiler 等目录已建立边界，其中除明确列出的契约与 Benchmark 领域类型外仍是骨架，不代表对应运行能力已经实现。
...
| `scripts/dev.ps1` | Web、API、Worker 本地开发统一入口的预留脚本。 |
...
| 路径 | 职责 |
|---|---|
| `apps/web/app/` | Next.js App Router 页面、布局和同源 API 代理；页面只做组装。 |
| `apps/web/components/` | 跨业务复用 UI 组件和设计系统封装，不承载领域流程。 |
| `apps/web/features/intake/` | 项目创建、导入校对、创意候选与 Brief 确认界面。 |
| `apps/web/features/workbench/` | CaseFile 对象树、详情、时间线、搜索与命令面板。 |
| `apps/web/features/quality/` | ValidationIssue 定位、修复建议和 PatchCandidate 审阅。 |
...
| `apps/web/lib/` | 生成 Client 配置、Query Client、SSE 和格式化器等无 UI 基础设施。 |
| `apps/web/store/` | 浏览器本地编辑器状态；服务端数据不得复制到这里。 |
| `apps/web/tests/` | 前端单元与组件测试。 |
