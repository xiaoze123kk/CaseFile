...
| `apps/web/features/analyst-workbench/analyst-workbench.tsx`、`analyst-workbench.module.css`、`prototype-relay.module.css`、`analyst-fixture.ts` | `/demo` 单页分析师工作台的种子驱动交互、石墨纸＋档案铜视觉与确定性 Fixture；案名、对象、事件、问题、来源、关系、推理路径、卷宗、抽屉与审计随候选变化，同时保留五区布局（左侧对象目录、右侧上下文检查器与时间线页的事件序列均可经分隔条拖拽调整宽度）、多视图同步、补丁审批、命令面板、响应式分区，以及顶栏入口的卷宗统筹 Agent 对话栏（右侧滑出，按种子数据确定性生成全卷宗体检/证据链摘要/候选解释对比/导出前检查回复，可自由输入指令）；卷宗编辑器与导出预览均铺满主画布：卷宗编辑器按选中事件对象推导字段（发生时间/地点/摘要、参与人物、关联地点/证据、候选假设、引用来源），页边备注动态显示引用计数与关联验证问题；导出预览两栏排版（封面 + 发布门禁）放大铺满；编译中心视图提供小说/剧本（剧本杀手册）/互动脚本/作者卷宗/测试材料五类编译目标，预览由种子数据确定性生成，编译按钮受验证门禁控制（存在未解决问题时禁用）；主画布关系图与推理图均为铺满画布的可拖动节点图（关系图保留 fixture 初始坐标、推理图自动按路径分列布局，节点拖动时边实时跟随，拖动超过阈值不误触点击联动）；关系图、推理图与地图画布均带缩放控件（50%–250%，百分比按钮点击复位）与选择/平移双工具（选择模式下点击节点联动、拖动节点改位置，平移模式下抓手拖动画布，节点点击不联动）；事件序列不缩放，时间线页仅同步关系图缩放与平移；推理图按支持/竞争/已排除三种收束边与图例表达，证据与结论节点点击联动对象目录，画布下方提供逐路径推理表与关系表文字替代；接力条只表达预览稿、当前工作稿和旧简报状态，采用按钮经 Provider 接真实候选采用。 |
