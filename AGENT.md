...
- 仓库采用前后端同仓、运行时分离的模块化单体。当前实现包含 PostgreSQL 数据底座、目标无关的 CaseFile v1 唯一机器契约、真实 SourceRecord → 可恢复 Brief Intake 追问/候选 → Brief 审阅/原子确认 → Agent → Validator → 不可变 Draft 候选 → 人工采用 → Snapshot 纵向切片；同一 BriefVersion 可重复生成候选，但项目始终只有一个当前可编辑 Draft。系统还包含基于最新完整 CaseFile 的持久多线程 Agent 对话、结构化建议逐项审阅、整批应用/撤销和过期门禁；`apps/web` 使用 React/Next.js 实现真实产品壳与“数字档案纸”界面。演示链路固定为 `/demo/intake` 建案、简报审阅/冻结与三稿决策，再进入 `/demo` 分析师工作台预览或采用；建案中心沿用正式创作模式的数字档案纸语言，分析师工作台保留独立的石墨纸与档案铜视觉。旧 `/demo/brief`、`/demo/workbench`、`/demo/reasoning`、`/demo/quality` 只重定向到工作台。演示状态只存在于 `DemoArchiveShell` 下的内存 Context，客户端路由切换保持、刷新或重新挂载即重置；不得请求真实 API、写入数据库或浏览器存储、挂载 Prototype Store，或混用真实 Workflow 状态，也不是第二套生产实现。Validator 当前覆盖结构、引用和确定性语义门禁，Simulation、Compiler 和任何 Target Adapter 后端仍未实现。
...
| `apps/web/app/demo/` | 分析师工作台与建案中心的独立演示路由壳；`/demo/intake` 挂载五阶段建案原型，`/demo` 挂载种子驱动工作台，旧演示子路径只负责重定向到 `/demo`，所有演示路由都不得导入真实 Workflow/API。 |
| `apps/web/components/app-shell.tsx`、`components/archive-shell.tsx`、`components/demo-archive-shell.tsx` | 按路由选择真实或演示产品壳；真实壳负责项目、Provider 和工作流导航，演示壳按路径标记两类视觉边界，并在共同祖先挂载 `DemoPrototypeProvider` 以支持不刷新的 `/demo/intake` ⇄ `/demo` 数据接力。 |
...
| `apps/web/features/demo-prototype/demo-prototype-provider.tsx` | 演示路由共享的纯内存 reducer/Context；保存建案步骤、简报审阅版本、候选生成、预览与显式采用状态，计算 `pending/current/stale`，Provider 重挂载时恢复默认样案，不得接 API、Workflow Store 或浏览器存储。 |
| `apps/web/features/analyst-workbench/analyst-workbench.tsx`、`analyst-workbench.module.css`、`prototype-relay.module.css`、`analyst-fixture.ts` | `/demo` 单页分析师工作台的种子驱动交互、石墨纸＋档案铜视觉与确定性 Fixture；案名、对象、事件、问题、来源、关系、卷宗、抽屉与审计随候选变化，同时保留五区布局、多视图同步、补丁审批、命令面板和响应式分区；接力条只表达预览稿、当前工作稿和旧简报状态。 |
| `apps/web/features/intake-prototype/intake-center-prototype.tsx`、`brief-review-stage.tsx`、`draft-candidates-stage.tsx`、`intake-prototype-model.ts` | `/demo/intake` 五阶段建案编排、简报审阅原子门禁、修订/冻结和三份候选决策；复用共享内存 Context，候选由纯函数 Fixture 确定生成，不得伪装为通用文本理解或导入真实 Workflow/API。 |
...
| `apps/web/tests/demo-prototype-state.test.tsx`、`intake-center-prototype.test.tsx` | 验证纯模型引用、冻结/修订/旧稿门禁、Provider 重挂载重置，以及五阶段建案、候选预览/采用和真实 Workflow/API 隔离。 |
