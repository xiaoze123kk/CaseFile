    ...
    __table_args__ = (
        ...
        CheckConstraint(
            (
                ...
                "compiler.input_manifest' AND schema_id = 'compiler.input-manifest"
                ".v1') OR (artifact_kind = 'narrative_ir' AND artifact_key = 'comp"
                "iler.narrative_ir' AND schema_id = 'compiler.narrative-ir.v1') OR"
                " (artifact_kind = 'novel_plan' AND artifact_key = 'compiler.novel"
                "_plan' AND schema_id = 'compiler.novel-plan.v1') OR (artifact_kin"
                "d = 'scene_plan' AND artifact_key = 'compiler.scene_plan' AND sch"
                "ema_id IN ('compiler.scene-plan.v1', 'compiler.scene-plan.v2')) O"
                "R (content_jsonb->>'schema_id' = schema_id AND ((artifact_kind = "
                "'scene_context' AND schema_id = 'compiler.prose-judge-checklist.v"
                "1' AND artifact_key = 'compiler.scene_context.' || (content_jsonb"
                "->>'scene_id')) OR (artifact_kind = 'scene_render' AND schema_id "
                "= 'compiler.scene-render.v1' AND content_jsonb->>'stage' IN ('wri"
                "ter','rewrite_1','rewrite_2','polished','accepted') AND artifact_"
                "key = 'compiler.scene_render.' || (content_jsonb->>'scene_id') ||"
                " '.' || (content_jsonb->>'stage')) OR (artifact_kind = 'validatio"
                "n_report' AND schema_id = 'compiler.prose-judge-report.v1' AND co"
                "ntent_jsonb->>'role' IN ('fidelity','adversarial','coherence','ar"
                "biter') AND artifact_key IN ('compiler.validation_report.' || (co"
                "ntent_jsonb->>'scene_id') || '.semantic_0.' || (content_jsonb->>'"
                "role'),'compiler.validation_report.' || (content_jsonb->>'scene_i"
                "d') || '.semantic_1.' || (content_jsonb->>'role'),'compiler.valid"
                "ation_report.' || (content_jsonb->>'scene_id') || '.semantic_2.' "
                "|| (content_jsonb->>'role'),'compiler.validation_report.' || (con"
                "tent_jsonb->>'scene_id') || '.preservation.' || (content_jsonb->>"
                "'role'))) OR (artifact_kind = 'validation_report' AND schema_id ="
                " 'compiler.prose-consensus-report.v1' AND artifact_key IN ('compi"
                "ler.validation_report.' || (content_jsonb->>'scene_id') || '.sema"
                "ntic_' || (content_jsonb->>'round') || '.consensus', 'compiler.va"
                "lidation_report.' || (content_jsonb->>'scene_id') || '.preservati"
                "on.consensus')) OR (artifact_kind = 'validation_report' AND schem"
                "a_id = 'compiler.prose-quality-report.v1' AND ((content_jsonb->>'"
                "report_kind' = 'findings' AND artifact_key = 'compiler.validation"
                "_report.' || (content_jsonb->>'scene_id') || '.quality.findings')"
                " OR (content_jsonb->>'report_kind' = 'pairwise' AND ((content_jso"
                "nb->'position_mapping'->>'a' = 'original' AND artifact_key = 'com"
                "piler.validation_report.' || (content_jsonb->>'scene_id') || '.qu"
                "ality.pairwise.original_first') OR (content_jsonb->'position_mapp"
                "ing'->>'a' = 'polished' AND artifact_key = 'compiler.validation_r"
                "eport.' || (content_jsonb->>'scene_id') || '.quality.pairwise.pol"
                "ished_first'))))) OR (artifact_kind = 'novel_candidate' AND artif"
                "act_key = 'compiler.novel_candidate' AND schema_id = 'compiler.no"
                "vel-candidate.v1') OR (artifact_kind = 'compile_manifest' AND art"
                "ifact_key = 'compiler.compile_manifest' AND schema_id = 'compiler"
                ".compile-manifest.v1')))), false)) OR ((artifact_kind IN ('scene_"
                "render','validation_report','compile_manifest')) AND artifact_key"
                " ~ '\\.attempt_[1-9][0-9]*$' AND COALESCE(((artifact_kind = 'input"
                "_manifest' AND regexp_replace(artifact_key, '\\.attempt_[1-9][0-9]"
                "*$', '') = 'compiler.input_manifest' AND schema_id = 'compiler.in"
                "put-manifest.v1') OR (artifact_kind = 'narrative_ir' AND regexp_r"
                "eplace(artifact_key, '\\.attempt_[1-9][0-9]*$', '') = 'compiler.na"
                "rrative_ir' AND schema_id = 'compiler.narrative-ir.v1') OR (artif"
                "act_kind = 'novel_plan' AND regexp_replace(artifact_key, '\\.attem"
                "pt_[1-9][0-9]*$', '') = 'compiler.novel_plan' AND schema_id = 'co"
                "mpiler.novel-plan.v1') OR (artifact_kind = 'scene_plan' AND regex"
                "p_replace(artifact_key, '\\.attempt_[1-9][0-9]*$', '') = 'compiler"
                ".scene_plan' AND schema_id IN ('compiler.scene-plan.v1', 'compile"
                "r.scene-plan.v2')) OR (content_jsonb->>'schema_id' = schema_id AN"
                "D ((artifact_kind = 'scene_context' AND schema_id = 'compiler.pro"
                "se-judge-checklist.v1' AND regexp_replace(artifact_key, '\\.attemp"
                "t_[1-9][0-9]*$', '') = 'compiler.scene_context.' || (content_json"
                "b->>'scene_id')) OR (artifact_kind = 'scene_render' AND schema_id"
                " = 'compiler.scene-render.v1' AND content_jsonb->>'stage' IN ('wr"
                "iter','rewrite_1','rewrite_2','polished','accepted') AND regexp_r"
                "eplace(artifact_key, '\\.attempt_[1-9][0-9]*$', '') = 'compiler.sc"
                "ene_render.' || (content_jsonb->>'scene_id') || '.' || (content_j"
                "sonb->>'stage')) OR (artifact_kind = 'validation_report' AND sche"
                "ma_id = 'compiler.prose-judge-report.v1' AND content_jsonb->>'rol"
                "e' IN ('fidelity','adversarial','coherence','arbiter') AND regexp"
                "_replace(artifact_key, '\\.attempt_[1-9][0-9]*$', '') IN ('compile"
                "r.validation_report.' || (content_jsonb->>'scene_id') || '.semant"
                "ic_0.' || (content_jsonb->>'role'),'compiler.validation_report.' "
                "|| (content_jsonb->>'scene_id') || '.semantic_1.' || (content_jso"
                "nb->>'role'),'compiler.validation_report.' || (content_jsonb->>'s"
                "cene_id') || '.semantic_2.' || (content_jsonb->>'role'),'compiler"
                ".validation_report.' || (content_jsonb->>'scene_id') || '.preserv"
                "ation.' || (content_jsonb->>'role'))) OR (artifact_kind = 'valida"
                "tion_report' AND schema_id = 'compiler.prose-consensus-report.v1'"
                " AND regexp_replace(artifact_key, '\\.attempt_[1-9][0-9]*$', '') I"
                "N ('compiler.validation_report.' || (content_jsonb->>'scene_id') "
                "|| '.semantic_' || (content_jsonb->>'round') || '.consensus', 'co"
                "mpiler.validation_report.' || (content_jsonb->>'scene_id') || '.p"
                "reservation.consensus')) OR (artifact_kind = 'validation_report' "
                "AND schema_id = 'compiler.prose-quality-report.v1' AND ((content_"
                "jsonb->>'report_kind' = 'findings' AND regexp_replace(artifact_ke"
                "y, '\\.attempt_[1-9][0-9]*$', '') = 'compiler.validation_report.' "
                "|| (content_jsonb->>'scene_id') || '.quality.findings') OR (conte"
                "nt_jsonb->>'report_kind' = 'pairwise' AND ((content_jsonb->'posit"
                "ion_mapping'->>'a' = 'original' AND regexp_replace(artifact_key, "
                "'\\.attempt_[1-9][0-9]*$', '') = 'compiler.validation_report.' || "
                "(content_jsonb->>'scene_id') || '.quality.pairwise.original_first"
                "') OR (content_jsonb->'position_mapping'->>'a' = 'polished' AND r"
                "egexp_replace(artifact_key, '\\.attempt_[1-9][0-9]*$', '') = 'comp"
                "iler.validation_report.' || (content_jsonb->>'scene_id') || '.qua"
                "lity.pairwise.polished_first'))))) OR (artifact_kind = 'novel_can"
                "didate' AND regexp_replace(artifact_key, '\\.attempt_[1-9][0-9]*$'"
                ", '') = 'compiler.novel_candidate' AND schema_id = 'compiler.nove"
                "l-candidate.v1') OR (artifact_kind = 'compile_manifest' AND regex"
                "p_replace(artifact_key, '\\.attempt_[1-9][0-9]*$', '') = 'compiler"
                ".compile_manifest' AND schema_id = 'compiler.compile-manifest.v1'"
                ")))), false)) OR COALESCE((artifact_kind = 'validation_report' AN"
                "D schema_id = 'compiler.prose-continuity-review.v1' AND content_j"
