    ...
    def simulate_patch_operation_batch(
        ...
        *,
        ...
        try:
            for ordinal, operation in enumerate(operations, start=1):
                ...
                if (
                    ...
                    and revisions.get(operation.object_id)
                    != operation.expected_object_revision
                ):
        ...
        deterministic_new = [
            final_by_key[key]
            for key in new
            if final_by_key[key].kind == "deterministic"
        ]
        ...
        new_same_or_higher = any(
            SEVERITY_RANK[item.severity] >= baseline_max
            for item in deterministic_new
        ) if baseline_max else bool(deterministic_new)
        can_apply = not (
        ...
        for issue in [*raw_errors, *semantic_errors]:
            ...
            findings.append(
                VerificationFinding(
                    ...
                    suggested_fix=(
                        str(issue["fix_hint"]) if issue.get("fix_hint") else None
                    ),
                    rule_code=code,
                    draft_revision=self.draft_revision,
                    refs=refs,
