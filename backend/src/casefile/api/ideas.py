...
def ideas_router() -> APIRouter:
    ...
    def list_ideas(project_id: int, actor: ActorDependency, session: SessionDependency) -> dict[str, Any]:
        return IdeaService(session).list(actor, project_id)

    @router.post("/projects/{project_id}/ideas/generate", status_code=201)
    def generate_ideas(project_id: int, actor: ActorDependency, session: SessionDependency) -> dict[str, Any]:
        return IdeaService(session).create_generation_task(actor, project_id)

    @router.post("/projects/{project_id}/ideas/{idea_id}/bookmark")
    def bookmark_idea(project_id: int, idea_id: int, actor: ActorDependency, session: SessionDependency) -> dict[str, Any]:
        return IdeaService(session).bookmark(actor, project_id, idea_id)

    @router.post("/projects/{project_id}/ideas/{idea_id}/archive")
    def archive_idea(project_id: int, idea_id: int, actor: ActorDependency, session: SessionDependency) -> dict[str, Any]:
        return IdeaService(session).archive(actor, project_id, idea_id)

    @router.post("/projects/{project_id}/ideas/{idea_id}/select")
    def select_idea(project_id: int, idea_id: int, actor: ActorDependency, session: SessionDependency) -> dict[str, Any]:
        return IdeaService(session).select(actor, project_id, idea_id)

    @router.post("/projects/{project_id}/ideas/{idea_id}/regenerate")
    def regenerate_idea(project_id: int, idea_id: int, actor: ActorDependency, session: SessionDependency) -> dict[str, Any]:
