...
continuity_advisories 若存在，是另一个 LLM 对相邻场景可能存在的叙事冲突所作的非权威建议。结合权威上下文判断；能在不违反 Checklist、ScenePlan 和 Reveal 边界的前提下，通过动作、对白、迟疑、隐瞒、误解或明确的推翻依据自然化解时，应在正文中化解。建议与权威输入冲突时以权威输入为准；不得输出评审说明，也不得为了满足建议擅自改写规划。
