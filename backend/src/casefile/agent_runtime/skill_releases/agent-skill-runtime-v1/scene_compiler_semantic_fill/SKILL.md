---
name: scene-compiler-semantic-fill
description: Stable execution method for scene_compiler_semantic_fill.
---
...
引用权限分为两层：object_catalog 只证明 ObjectRef 存在；actor 权限必须另由当前 Scene 的 actor_allowlist（与 participant_refs 完全相同）授权，target 或嵌套状态引用仍须遵守各自约束，不代表它可以作为顶层 Beat provenance。每个 Beat 的顶层 basis_refs 必须非空，且每一项都必须逐项来自该 Beat 所属 Scene 的 beat_basis_allowlist；即使某个引用出现在 object_catalog 或 state_seed 中，只要不在该 Scene 的 beat_basis_allowlist，就不得写入 Beat.basis_refs。
...
depends_on 是严格的 Scene-local 引用：每一项只能引用同一个 scene_fill.beats 数组中严格更早出现的 local_key。即使另一个 Scene 位于同一 batch 且已经输出，也绝对不能在当前 Scene 的 depends_on 中引用它的 Beat local_key。跨 Scene 依赖已经由服务端锁定的 prerequisite_scene_ids 表达，不得在 Beat depends_on 中重复、转写或推断。当前 Scene 的第一个 Beat 必须使用空 depends_on。

knowledge_transitions 必须逐条服从 inbound_state.knowledge_operation_constraints。先以 subject_ref 与 object_ref 精确匹配一条约束，再保证 operation 是该约束 allowed_operations 中明确列出的值；不得自行扩大集合。若 allowed_operations 不含 misbelieve，绝对不能让该主体对该对象执行 misbelieve，尤其不能把已经位于 knows_refs 的已知事实改成 false belief。若场景不需要知识变化，或没有符合白名单的操作，保持 knowledge_transitions 为空，不得为了丰富叙事而伪造变化。
...
每个 Scene 的 intent 是该场景的叙事目标，必须据此细化，不得把相邻场景的行动、回应提前搬入当前 Scene。每个 Beat.actor_refs 必须逐项来自当前 Scene.actor_allowlist；目录、全章参与者、被谈论人物、证据涉及人物都不等于在场行动者。没有合适的行动者时不得自行扩充参与者。
若提供 repair_context，只纠正列出的违规及其必要关联；保留原场景身份、顺序、已合规内容和所有义务。返回完整当前批次，不得申请更多修复。
