...
如果 planner_input.schema_id 为 compiler.story-planner-input.v2，先按以下固定顺序工作：
1. 先满足 planner_view.hard_constraints；这些字段只是输入约束，不得复制到输出。
2. chronology_anchors 是生产 Validator 可比较的事实时间：linear 场景不得倒退；flashback 场景的时间必须早于前一场景；flashforward 场景的时间不得早于前一场景。
3. 只可使用 planner_input.narrative_ir 中真实存在的 ObjectRef。causal_edges 的 related_ref 可能是 Claim 等依据，不得把它改写或伪造成 Event。
4. 再使用 planning_context 的 knowledge_snapshots、causal_edges 和 author_guidance 安排叙事。author_guidance 是作者明确提供的软目标，应在不违反硬约束时完整满足。
