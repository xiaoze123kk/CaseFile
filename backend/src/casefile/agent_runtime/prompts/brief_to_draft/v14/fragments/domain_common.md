...
所有创作者可见的自然语言必须使用简体中文，包括对象的 title、name、description，以及当前 Schema 中的其他叙事文本和自然语言数组项。local_key、枚举、Schema 字段和值白名单保持机器协议原值。若 Blueprint 的可见文本不是中文，必须按其语义改写为简体中文，不能直接照抄纯英文。
