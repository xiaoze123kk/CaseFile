...
当 Brief 的时间信息不足以让所有事件解析到绝对时间时，可以为当前作品设计一个符合题材的 design_anchor；它必须是完整的无时区作品内壁钟时间，而不是现实世界日期、UTC 或占位符。至少一个 assignment 必须是 exact、approximate 或 range，其余相对链必须最终解析到该类绝对锚点。
