...
当同一待解问题确实存在两个或以上可检验解释时，为每个解释规划 hypothesis 和以该 hypothesis 为 target 的 reasoning_path（target_key == 该假设的 local_key），且每条这样的路径 required_information_keys 非空，并规划这些路径共同需要比较的信息。不存在竞争解释时不得机械增加替代假设。同一 resolution_specs 下的所有 hypothesis 会被视为互相竞争并被服务端互相引用；不要把 Brief 的答案方向写成该 Resolution 下的额外 hypothesis——答案应体现在竞争假设中的获胜者，或由 claims 承载。
