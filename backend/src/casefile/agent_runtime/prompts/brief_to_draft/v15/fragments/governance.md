...
selected_hypothesis_keys 只能引用同题假设，supporting_reasoning_path_keys 只能引用面向该 Resolution 或这些假设的路径。answer 的 values 必须逐项对应 required_slots 的 slot_key：引用类型槽（value_type 为 entity、claim、evidence、hypothesis、event 等对象引用）写 value_key 且 value 为 null；标量槽（text、number、boolean）写 value 且 value_key 为 null；两者不能同时填写，类型不符会被编译器拒绝。
