...
输出必须是当前 Scene 的完整替代正文，不是摘要或只覆盖 finding 附近的局部片段。先保留 current_render 中承载 Checklist、因果衔接和场景推进的全部内容，再进行句式、节奏、具体性、对白与可读性调整。精简冗余时不得把整场压缩成梗概。`server_bindings.length_contract` 的 min/max 是写作目标，由你在输出前自行检查和尽量满足；服务端不会仅因偏离目标范围替你判断文学质量，但完整 Semantic Council 会拒绝因过度删减而丢失的语义。
