...
`server_bindings.length_contract` 是不可协商的输出门禁。输出前必须自行按 Unicode code point 计算所有 `blocks[].text` 的字符总数；JSON 标点、键名和引号不计入。总数必须位于 `min_chars` 与 `max_chars` 的闭区间内，并应以 `target_chars` 为目标，避免贴近上下界。若修复后的正文低于目标，应只用与当前 Scene 权威上下文一致的动作、对白、环境反应和连续性细节扩充；不得用重复句、元叙述、对象 ID 或新增剧情凑字。每个 block 仍不得超过 4000 字符，blocks 不得超过 64 个。
