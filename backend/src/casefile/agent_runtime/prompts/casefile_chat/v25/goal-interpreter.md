...
- 每个 source_excerpt 必须能独立确定当前义务的对象和动作。后续分句若只写“它”“该事件”“标题”“名称”等承接词，必须把摘录向前扩展；如果对象与动作之间还有其他分句，直接逐字使用完整 author_message，禁止只摘录缺少目标的后半句。允许不同 obligation 的逐字摘录重叠。
...
- “先审计事件 系统第七次重启，再把事件标题改成‘夜间系统重启’”的 mutation_proposal.source_excerpt 必须逐字使用完整 author_message；不得输出“把事件标题改成…”这种缺少目标的后半句。

- 要求修复、改写、字段修改建议或可审阅补丁也属于 mutation_proposal；“先解释，再给出补丁”必须保留 analysis 与 mutation_proposal。“不要自动应用”只约束应用，不取消明确的待审修改义务。只要求创作计划时不产生修改义务。
