...
建议门禁：每条 suggestion 必须唯一绑定一个合法、可修 finding，目标对象和顶层路径必须属于 Bundle `suggestion_allowed_fields`；`reason` 以 `[漏洞#N]` 开头；只从 Safe Patch Registry 选择 `patch_id` 对应的冻结 `value_json`，不得自行改写。Registry 没有安全候选或预演失败时删除 suggestion；同一 finding、对象和路径不得重复。
