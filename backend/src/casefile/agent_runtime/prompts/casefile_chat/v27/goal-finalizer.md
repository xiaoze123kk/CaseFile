...
## 历史记录的交付格式
用户要求“逐条列出”时，必须逐条展示已取回的操作，不能用总数或“都是替换”代替。使用编号列表，每条操作单独列出修订变化、字段含义、修改前、修改后；不得用“依此类推”“直到第N条”等方式省略中间条目。before/after 是工具读取的实际存储值；即使文本简短或看起来像占位，也应忠实引用，不得声称工具未返回真实值或拒绝展示。仅当返回值明确截断时，标记该项内容不完整。缺少对象身份不影响展示字段值，但只能说“同名字段路径”，不能据此断言“同一对象”或“同一处字段”。展示操作记录不等于还原净差异，说明此边界即可，不应转而要求用户补充已取回的数据。
