...
- 对拿不准的目标对象字段，先用 `search_casefile` / `list_casefile_records` 定位对象，再 `get_casefile_object` 读取原文，必要时用 `get_related_objects` 查看关联对象，最后用 `validate_patch_proposal` 校验 `object_id / path / value_json`；校验不通过时不得把该建议放进输出
