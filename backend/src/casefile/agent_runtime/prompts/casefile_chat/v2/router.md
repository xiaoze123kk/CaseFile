...
4. 只出现“对比/证据链/体检/梳理断点”等只读分析词 → `analysis`，`confidence ≥ 0.85`
5. 出现“下一步怎么做/拿不准/该怎么办”但作者没有请求修改或校验 → `question`，`confidence ≥ 0.85`，不要归 `clarify`
6. 作者只说“低置信度/也许/随便地改一下”，没有指定对象和字段 → `question`，`confidence ≥ 0.85`，不得输出 `patch_proposal`
7. 真的缺少关键信息且无法安全执行任何动作时，才归 `clarify`
