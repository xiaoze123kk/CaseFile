...
- `retrieve_thread_evidence` 只能传 `context_dashboard.recoverable_evidence_ids` 里实际存在的证据 ID；它只读恢复已压缩区间的原始消息作为事实依据，不删除、不改写任何线程证据
- `request_thread_compaction` 只是请求，不改变本轮预算：是否压缩由系统在本轮回复结束后裁决，调用后仍须完成当前回答
...
引用完整性硬约束（最终答案必须满足）：
- 输出前逐项核对正文：正文中实质讨论的每个对象、事件、验证问题，其真实 ID 必须已写入对应的引用槽；正文实质讨论但引用槽漏填，视为任务失败
- ID 白名单：`referenced_object_ids` 与 `referenced_event_ids` 只能填写 `casefile.records`、`focus_objects.objects`、`focus_objects.neighbors` 或本轮工具结果中实际存在的 ID；`referenced_validation_issue_ids` 只能填写 `validation_issues` 中实际存在的 `issue_id`
- 禁止填写 `src_*`、`clm_*` 等 source/brief 内部 ID，禁止填写任何凭记忆猜测的 ID
- 正例：正文实质回答“林研究员负责什么” → 若 `casefile.records` 或 `focus_objects` 中存在 `ent_researcher` 这一记录，`referenced_object_ids` 必须包含 `ent_researcher`
- 负例：正文只是顺带提及一个名字但没有围绕它展开实质回答 → 不填；正文没有实质讨论任何对象/事件 → 对应引用数组保持空
- 不得为了填满数组而扩充正文或新增对未讨论对象的引用；引用数组只描述正文已经实质讨论的内容

输出规则：
- 生成最终结构化结果前，先执行引用完整性自检：重新阅读正文，逐项确认每个实质讨论对象/事件/验证问题的 ID 已进入对应引用槽，再返回结果
