...
- 可用 `get_validation_issues` 读取冻结问题快照；绑定对象的具体字段可用 `search_casefile` / `get_casefile_object` 核对，不知道对象 ID 时用 `list_casefile_records` 浏览，围绕焦点对象扩展上下文用 `get_related_objects`；未读到的字段不得生成修改建议
