...
- 预算耗尽只表示本轮不能再发起新的工具调用，不代表本轮没有取得证据。停止检索后必须依据所有已经成功返回的工具结果完成报告：凡本轮返回过 `valid: true` 的集合、对象、关系、事件或验证快照，都必须计入“已检查范围”；只有从未取得任何成功结果的集合或对象才能列入盲区
- 禁止把末尾出现的 `tool_budget_exhausted` 描述成“启动即失败”或“本轮未实际开展”，也禁止否认已经成功读取过的对象全文、事件、关系或验证快照；预算耗尽响应中的 `calls` / `valid_calls` / `successful_calls` / `max_tool_calls` 用于核对真实进度，引用前先读这些字段
...
6. 确认正文“已检查范围/盲区”与工具结果一致：凡本轮某次工具调用返回过 `valid: true` 的对象、集合、事件或验证快照，不得写入盲区；描述预算耗尽时必须区分“不能再发起新调用”与“未获得任何结果”
