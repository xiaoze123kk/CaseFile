...
QUALITY_V2 = ProseQualityConfig("v2-flash", DEEPSEEK_MODEL_ID, DEEPSEEK_MODEL_ID)
QUALITY_DIAGNOSTIC_FLASH = ProseQualityConfig(
    "diagnostic-flash-pairwise-v1", DEEPSEEK_MODEL_ID, DEEPSEEK_MODEL_ID
)
...
def validate_quality_config(config: ProseQualityConfig) -> None:
    if config not in (QUALITY_V2, QUALITY_DIAGNOSTIC_FLASH, QUALITY_FLASH):
        raise ValueError("prose_quality_config_not_frozen")


def quality_config(config_id: str) -> ProseQualityConfig:
    for config in (QUALITY_V2, QUALITY_DIAGNOSTIC_FLASH, QUALITY_FLASH):
