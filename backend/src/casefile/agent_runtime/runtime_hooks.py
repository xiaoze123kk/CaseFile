"""Typed, invocation-local hooks shared by Agent runtimes.
...
SyncHookHandler = Callable[[HookInput], HookResult]


class SyncHookDispatcher:
    """Pure synchronous boundaries; preserve domain exceptions for bounded repair.

    Unlike async hooks, these callbacks must not perform blocking I/O. The timeout
    is checked on return; it cannot preempt a synchronous Python callback.
    Records contain metadata only, never input, output, or exception text.
    """

    def __init__(self, handlers: Mapping[tuple[str, str], SyncHookHandler]) -> None:
        self.handlers = MappingProxyType(dict(handlers))
    ...
    def dispatch(
        ...
        *,
        ...
        # Validate the complete policy before executing even the first callback.
        for binding in bindings:
            if (binding.handler_id, binding.version) not in self.handlers:
                raise HookExecutionError(f"Unknown hook: {binding.handler_id}@{binding.version}")
        for binding in dict.fromkeys(bindings):
            ...
            try:
                ...
                result = self.handlers[(binding.handler_id, binding.version)](snapshot)
                if not isinstance(result, HookResult):
                    raise TypeError("Hook must return HookResult")
                HookDispatcher._validate_result(event.event, result)
                # Sync boundaries only validate or observe. Never silently drop
                # repair findings or resource requests that the caller cannot use.
                if result.resources or result.issues:
                    raise HookExecutionError("Sync hooks must validate or observe")
                if result.block_reason:
                    raise HookExecutionError(result.block_reason)
                if perf_counter() - started > binding.timeout:
                    raise TimeoutError("Synchronous hook exceeded its execution budget")
