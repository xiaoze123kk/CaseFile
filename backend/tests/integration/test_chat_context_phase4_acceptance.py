...
v5 (structured logic_audit rollout) is the accepted default, pairing
``casefile-chat-context-v5`` with the v9 prompt and v4 toolset. The v4 opt-in
test still runs when
...
def test_phase4_v5_rollout_binds_v9_v4_toolset_and_structured_audit(
    ...
    rollout = os.environ.get("CASEFILE_CHAT_CONTEXT_ROLLOUT")
    if rollout not in (None, ROLLOUT_V5):
