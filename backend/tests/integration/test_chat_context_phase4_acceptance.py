...
default, pairing ``casefile-chat-context-v6`` with the v10 prompt and v4
...
def test_phase4_v6_rollout_binds_v10_v4_toolset_and_hardened_audit(
    workflow_database: tuple[Engine, int, str],
) -> None:
    rollout = os.environ.get("CASEFILE_CHAT_CONTEXT_ROLLOUT")
    ...
    with (
        ...
        with factory() as session:
            ...
            assert first_row.prompt_version == CHAT_CONTEXT_PROMPT_V7_VERSION
            assert first_row.toolset_version == CHAT_TOOLSET_V4_VERSION
            assert first_row.input_jsonb.get("context_policy_version") == ROLLOUT_V6

        assert len(provider.requests) == 1
        request = provider.requests[0]
        assert request.prompt_version == CHAT_CONTEXT_PROMPT_V7_VERSION
        assert request.toolset_version == CHAT_TOOLSET_V4_VERSION
