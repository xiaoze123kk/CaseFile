...
v4 (logic_audit rollout) is the accepted default. The v3 opt-in test still runs
when ``CASEFILE_CHAT_CONTEXT_ROLLOUT=casefile-chat-context-v3``. Both use the
deterministic FakeProvider and lowered compaction thresholds so one exchange
produces the same rolling compaction behavior as Phase 3, paired with the
matching prompt package and toolset.
"""
...
def test_phase4_v4_rollout_binds_v8_v4_toolset_and_full_audit_snapshot(
    ...
    if rollout not in (None, ROLLOUT_V4):
