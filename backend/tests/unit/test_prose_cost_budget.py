...
def test_revision_count_excludes_judge_and_quality_stages():
    rows = [{"role": "rewriter", "stage": stage} for stage in ("rewriter", "fidelity", "quality")]
    assert summarize_calls(rows)["revision_calls"] == 1
